        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    ...
                    <form id="eventForm" method="POST" action="{{ route('event.update', $event->id) }}"
                            ...
                            <div class="col col-md-6">
                                <div class="form-group">
                                    ...
                                    <select name="type" class="form-control" required>
                                        ...
                                        <option value="Dangerous situation"
                                            {{ $event->type == 'Dangerous situation' ? 'selected' : '' }}>Situation
                                            Dangereuse</option>
                                        <option value="Incident" {{ $event->type == 'Incident' ? 'selected' : '' }}>
                                            Incident</option>
                                        <option value="Near miss" {{ $event->type == 'Near miss' ? 'selected' : '' }}>
                                            Presque Accident</option>
                                        <option value="Work accident"
                                            {{ $event->type == 'Work accident' ? 'selected' : '' }}>Accident du Travail
                                            (AT)</option>
                                        <option value="Occupational illness"
                                            {{ $event->type == 'Occupational illness' ? 'selected' : '' }}>Maladie
                                            Professionnelle (MP)</option>
                                        <option value="Impact environnemental"
                                            {{ $event->type == 'Impact environnemental' ? 'selected' : '' }}>Impact
                                            environnemental</option>
                                    </select>
                                </div>
                        ...
                        <div class="mb-2">
                            @if ($event->path)
                                <img src="{{ asset('storage/' . $event->path) }}" alt="Uploaded Image" width="200"
                        ...
                        <div class="form-group">
                            ...
                            <div id="securite_acces" class="category-content">
                                <div class="checkbox-group">
                                    @foreach (['Chute de plain-pied ou escalier', 'Chute dans trémie sans protection ou mal protégée', 'Chute de hauteur', 'Chute d’objet', 'Franchissement d’un balisage ou d’un garde-corps', 'Cheminement non sécurisé (manque de visibilité, obstacle, etc.)'] as $item)
                                        <div class="checkbox-item">
                                            <input type="checkbox" name="analyse[securite_acces][]"
                                                value="{{ $item }}"
                            ...
                            <div id="materiel_securite" class="category-content">
                                <div class="checkbox-group">
                                    @foreach (['Matériel de sécurité non vérifié', 'Matériel de sécurité inadapté', 'Matériel de sécurité indisponible', 'Non respect d’une consigne de sécurité', 'Non port des EPI', 'Non utilisation d’EPC'] as $item)
                                        <div class="checkbox-item">
                                            <input type="checkbox" name="analyse[materiel_securite][]"
                                                value="{{ $item }}"
                            ...
                            <div id="info_risques" class="category-content">
                                <div class="checkbox-group">
                                    @foreach (['Absence de PdP ou de PPSPS', 'Non communication du PdP ou du PPSPS au collaborateur', 'Information sur les risques potentiels incomplète ou absente', 'Absence d’accueil sécurité sur le site'] as $item)
                                        <div class="checkbox-item">
                                            <input type="checkbox" name="analyse[info_risques][]"
                                                value="{{ $item }}"
                            ...
                            <div id="ambiances" class="category-content">
                                <div class="checkbox-group">
                                    @foreach (['Produits toxiques, nocifs', 'Produits corrosifs, irritants', 'Risque électrique', 'Risque d’incendie, d’explosion', 'Risques liés à la coactivité', 'Modification des risques en cours de mission', 'Conditions d’hygiène dans les locaux'] as $item)
                                        <div class="checkbox-item">
                                            <input type="checkbox" name="analyse[ambiances][]"
                                                value="{{ $item }}"
                            ...
                            <div class="category-title" onclick="toggleCategory('formation')">Formation sécurité /
                                Habilitations / Autorisations</div>
                            <div id="formation" class="category-content">
                                <div class="checkbox-group">
                                    @foreach (['Formation sécurité insuffisante ou absente', 'Intervention d’un collaborateur non habilité', 'Habilitation périmée, à réexaminée'] as $item)
                                        <div class="checkbox-item">
                                            <input type="checkbox" name="analyse[formation][]"
                                                value="{{ $item }}"
                            ...
                            <div class="col col-md-6">
                                <div class="form-group">
                                    ...
                                    <select name="frequence" class="form-control" required>
                                        <option value="1" {{ $event->frequence == 1 ? 'selected' : '' }}>Faible
                                            (= 1 fois/an)</option>
                                        <option value="2" {{ $event->frequence == 2 ? 'selected' : '' }}>Moyenne
                                            (= 1 fois/mois)</option>
                                        <option value="3" {{ $event->frequence == 3 ? 'selected' : '' }}>Grande
                                            (> 1 fois/mois)</option>
                                        <option value="4" {{ $event->frequence == 4 ? 'selected' : '' }}>Grande
                                            (> 1 fois/semaine)</option>
                                    </select>
                                </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="gravite" class="form-control" required>
                                        <option value="1" {{ $event->gravite == 1 ? 'selected' : '' }}>Gêne ou
                                            dommage léger</option>
                                        <option value="2" {{ $event->gravite == 2 ? 'selected' : '' }}>Blessure
                                            légère</option>
                                        <option value="3" {{ $event->gravite == 3 ? 'selected' : '' }}>Blessure
                                            grave</option>
                                        <option value="4" {{ $event->gravite == 4 ? 'selected' : '' }}>Blessure
                                            mortelle</option>
                                    </select>
                                </div>
                            </div>
                        ...
                        <div class="form-group">
                            ...
                            <div class="checkbox-group">
                                @foreach (['Information', 'Organisation', 'Equipement', 'Autre'] as $mesure)
                                    <div class="checkbox-item">
                                        <input type="checkbox"
                                            id="{{ $mesure == 'Autre' ? 'autre_input' : 'mesure_' . strtolower($mesure) }}"
                                            name="mesures[]" value="{{ $mesure }}"
                                            {{ in_array($mesure, $mesures) ? 'checked' : '' }}>
                                        <label>{{ $mesure == 'Equipement' ? 'Equipement de sécurité' : $mesure }}</label>
                                    </div>
                                @endforeach
                            </div>
                            <div class="form-group autre_input_show"
                                style="{{ $event->autre_checkbox ? '' : 'display: none;' }}">
                                <input type="text" name="autre_checkbox" id="autre_checkbox"
                                    value="{{ $event->autre_checkbox ?? '' }}" class="form-control "
                                        ...
                                        @foreach ($actions as $index => $action)
                                            <tr>
                                                <td><input type="text"
                                                        ...
                                                        class="form-control" value="{{ $action['description'] ?? '' }}">
                                                </td>
                                                <td><input type="text"
                                                        name="actions[{{ $index }}][responsable]"
                                                        class="form-control" value="{{ $action['responsable'] ?? '' }}">
                                                </td>
                                                <td><input type="date" name="actions[{{ $index }}][delai]"
                                                        id="deadline{{ $index }}" class="form-control"
                                                        value="{{ $action['delai'] ?? '' }}"></td>
                                                <td>
                                                    <select name="actions[{{ $index }}][type]"
                                                        class="form-select">
                                                        <option value="Immediate"
                                                            {{ ($action['type'] ?? '') == 'Immediate' ? 'selected' : '' }}>
                                                            Immédiate (I)</option>
                                                        <option value="Corrective"
                                                            {{ ($action['type'] ?? '') == 'Corrective' ? 'selected' : '' }}>
                                                            Corrective (C)</option>
                                                        <option value="Preventive"
                                                            {{ ($action['type'] ?? '') == 'Preventive' ? 'selected' : '' }}>
                                                            Préventive (P)</option>
                                                    </select>
                                                </td>
                                                <td><button type="button" class="btn btn-danger remove-action"
                                                        {{ $index == 0 ? 'disabled' : '' }}>Remove</button></td>
                                            </tr>
                                        @endforeach
                                    </tbody>
                                </table>
        ...
        $(document).ready(function() {
            ...
            function calculateRisk() {
                ...
                if (risk <= 4) {
                    ...
                    message =
                        'Action urgente à entreprendre immédiatement (Réalisation arbre des causes systématiquement)';
